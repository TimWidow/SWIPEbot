    ...
    if resp.get('results'):
        if action == 'add_house':
            ...
            for item in resp['results']:
                if item.get('flat_count') > 0:  # If house doesnt have apartments - wont display this house
                    new_resp['results'].append(item)
            resp = new_resp
            if not resp.get('results'):
                return 'У вас нет домов с квартирами', keyboard([], action), False
...
async def go_to_house(message: types.Message, state: FSMContext):
    data = await state.get_data()
    error_text = 'Домов нет. Добавьте дом, преждем чем создавать объявление'
    ...
    if not status:
        ...
        keyboard_cor.close()
        return
    await message.answer('Выберите дом')
    if data.get('post_info'):
        await message.answer('Сейчас: {value}'.format(value=data['post_info']['house']))
...
async def go_to_flat(message: types.Message, state: FSMContext):
    ...
    if not data.get('create_house') or not data['create_house'].get('house'):
        await message.answer('Нужно выбрать дом')
        return
    params = {'house_pk': data['create_house']['house']}
    error_text = 'Квартир нет. Добавьте квартиру, прежде чем создавать объявление'
    text, keyboard_cor, status = await list_items(user_id=message.from_user.id,
                                                  url=REL_URLS['apartments_public'],
    ...
    if not status:
        ...
        keyboard_cor.close()
        return
    await message.answer('Выберите квартиру')
    if data.get('post_info'):
        await message.answer('Сейчас: {value}'.format(value=data['post_info']['flat']))
    await message.answer(text, reply_markup=await keyboard_cor)
...
async def go_to_price(message: types.Message, state: FSMContext):
    ...
    if data.get('post_info'):
        ...
        current_price = post_data.get('price', 'Не указано')
    await message.answer('Текущая цена: {price}\n' +
...
async def go_to_payment_option(message: types.Message, state: FSMContext):
    data = await state.get_data()
    if data.get('post_info'):
        await message.answer('Сейчас: {value}'.format(value=data['post_info']['payment']))
    await message.answer('Выберите метод оплаты',
...
async def go_to_communications(message: types.Message, state: FSMContext):
    data = await state.get_data()
    if data.get('post_info'):
        await message.answer('Сейчас: {value}'.format(value=data['post_info']['communication']))
    await message.answer('Выберите метод коммуникации', reply_markup=await get_communication_keyboard())
...
async def go_to_description(message: types.Message, state: FSMContext):
    ...
    post_data = data.get('create_post')
    if post_data:
        current_desc = post_data.get('description', 'Не указано')
        await message.answer('Текущее описание\n {desc}'.format(desc=current_desc))
    if data.get('post_info'):
        current_desc = data['post_info']['description'] or 'Не указано'
...
async def edit_post(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if not status:
        ...
        keyboard_cor.close()
        return
    await call.message.answer('Выберите дом\n' +
...
async def add_post(message: types.Message, state: FSMContext):
    ...
    await message.answer('Заполните форму для создания объявления',
                         reply_markup=keyboard)
...
async def add_house(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if resp:
        ...
        error_text = 'Квартир нет. Добавьте квартиру, преждем чем работать с объявлением'
        text, keyboard_cor, status = await list_items(user_id=call.from_user.id,
                                                      url=REL_URLS['apartments_public'],
                                                      deserializer=flat_des,
                                                      keyboard=get_item_for_create_post,
        ...
        if not status:
            ...
            keyboard_cor.close()
            return
        await call.message.answer('Выберите квартиру')
        if state_data.get('post_info'):
            text = 'Сейчас: {flat}'.format(flat=state_data['post_info']['flat'])
            await call.message.answer(text)
        if not text:
            await call.message.answer('В этом доме ещё нет квартир')
            keyboard_cor.close()
            return
        await call.message.answer(text, reply_markup=await keyboard_cor)
        await CreatePost.FLAT.set()
    else:
        await call.answer('Такого дома нет в системе. Выберите другой')
...
async def get_flat(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if resp:
        ...
        await call.message.answer('Выберите метод оплаты', reply_markup=await get_payment_options())
        if state_data.get('post_info'):
            await call.message.answer('Сейчас: {value}'.format(value=state_data['post_info']['payment']))
        await CreatePost.PAYMENT.set()
    else:
        await call.answer('Нет такой квартиры в системе. Выберите другую')
...
async def get_payment(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.answer('Метод оплаты добавлен')
    await call.message.answer('Введите цену')
    if state_data.get('post_info'):
        await call.message.answer('Сейчас {price}'.format(state_data['post_info']['price']))
    await CreatePost.PRICE.set()
...
async def get_price(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer('Цена добавлена')
        await message.answer('Выберите метод коммуникации', reply_markup=await get_communication_keyboard())
        if state_data.get('post_info'):
            await message.answer('Сейчас: {value}'.format(value=state_data['post_info']['communication']))
        await CreatePost.COMMUNICATION.set()
    except ValueError:
...
async def get_photo(message: types.Message, state: FSMContext):
    ...
    if not photos:
        await message.edit_text('Вы не добавили изображение')
        return
        ...
        with open(main_image.file_path, 'rb') as rb_image:
            ...
            if data.get('post_info'):
                ...
                if not resp.get('id'):
                    ...
                    await call.answer('Объявление изменено', show_alert=True)
                    data.pop('create_post')
                    await state.finish()
                    keyboard, path = await dispatcher('LEVEL_1', user_id=call.from_user.id)
                    data['path'] = path
                    await call.message.answer('Возврат на главное меню', reply_markup=keyboard)
                    await state.update_data(**data)
            else:
                resp, status = await Conn.post(REL_URLS['posts'], data=post_data, user_id=call.from_user.id)
                if status == 201:
                    await call.answer('Объявление создано', show_alert=True)
                    data.pop('create_post')
                    await state.finish()
                    keyboard, path = await dispatcher('LEVEL_1', user_id=call.from_user.id)
                    data['path'] = path
