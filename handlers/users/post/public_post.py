...
async def get_filter_list(message: Union[types.Message, types.CallbackQuery]):
    ...
    if text:
        ...
        return 'Фильтров нет', user_keyboards.get_keyboard_for_filter([])
...
async def handle_filters(message: Union[types.Message, types.CallbackQuery]):
    ...
    if isinstance(message, types.Message):
        ...
        try:
            ...
            await message.message.edit_text('Фильтров нет')
    else:
        keyboard_cor.close()


async def get_post(call: types.CallbackQuery, callback_data: dict,
                   keyboard_key: str, keyboard=None):
...
async def user_filters_callback(call: types.CallbackQuery):
...
async def filter_detail(call: types.CallbackQuery, callback_data: dict):
    ...
    if resp:
        ...
        await call.answer('Фильтра не найден', show_alert=True)
...
async def delete_filter(call: types.CallbackQuery, callback_data: dict):
    ...
    if status == 204:
        await call.answer('Фильтр удален', show_alert=True)
        await handle_filters(call)
    else:
...
async def public_post(message: types.Message, state: FSMContext):
    ...
    await message.answer('Список объявлений', reply_markup=keyboard)
...
async def list_post(call: types.CallbackQuery, callback_data: dict):
    ...
    await Conn.patch(url, data=data, user_id=call.from_user.id)
...
async def save_to_favorites(call: types.CallbackQuery, callback_data: dict):
    ...
    if status == 201:
        await call.answer(text='Добавлено', show_alert=True)
    elif status == 409:
        await call.answer(text='Это объявление уже в вашем списке избранного',
                          show_alert=True)
    else:
        log.error(resp)
...
async def complaint(call: types.CallbackQuery, callback_data: dict):
    ...
    if callback_data.get('type') != '_':
        ...
        if status == 201:
            ...
            await call.answer('Вы уже отправили жалобу.', show_alert=True)
        else:
            log.error(resp)
            await call.answer('Произошла ошибка. Повторите попытку')
    else:
        keyboard = await user_keyboards.get_post_complaint_types(post_pk=pk)
        await call.message.answer(text='Укажите причину жалобы', reply_markup=keyboard)
        await call.answer()
...
async def delete_promotion(call: types.CallbackQuery, callback_data: dict):
    ...
    if resp.get('results'):
        ...
        if status == 204:
            await call.answer('Продвижение убрано')
        else:
            await call.answer('Произошла ошибка')
    else:
        await call.answer('Для этой публикации продвижение не заказано')
