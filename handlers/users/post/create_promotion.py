...
async def go_to_phrase(message: types.Message):
    await message.answer('Выберите фразу', reply_markup=phrase_keyboard)
...
async def go_to_type(message: types.Message):
    ...
    await message.answer('Выберите тип продвижения',
...
async def add_promotion(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if resp.get('results'):
        await call.answer('Для этой публикации уже заказано продвижение')
        return

    await CreatePromotion.STARTER.set()
    keyboard, path = await dispatcher('LEVEL_3_ADD_PROMOTION', call.from_user.id)
    await call.message.answer('Заказ продвижения объявления\n' +
                              'Фраза: 50 грн', reply_markup=keyboard)
    await call.message.answer('Выберите специальную фразу', reply_markup=phrase_keyboard)
    await CreatePromotion.PHRASE.set()
...
async def add_promotion_phrase(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer('Выберите тип', reply_markup=await get_promotion_type_keyboard(resp.get('results')))
    text = ''
    for index, item in enumerate(resp.get('results'), start=1):
        prices[item['id']] = types.LabeledPrice(label=item['name'], amount=(int(item['price']) * 50) + 500)
        text += '{index}. {name}.\n Эффективность {efficiency}\n ' + 'Цена {price} грн.'.format(index=index,
                                                                                                name=item['name'],
                                                                                                efficiency=item[
                                                                                                    'efficiency'],
                                                                                                price=item['price'])
...
async def add_promotion_type(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer('Подтверждаете?', reply_markup=confirm_keyboard)
    await CreatePromotion.SAVE.set()
...
async def save_promotion(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if value:
        ...
        if status == 201:
            await call.answer('Необходимо оплатить заказ', show_alert=True)
            await call.message.answer('Можете использовать следующие данные тестовой платежной карты: \n' +
            ...
            await call.bot.send_invoice(
                ...
                title='Оплата заказа на продвижение объявления',
                description='Без оплаты продвижения не случится',
        ...
        await call.answer('Вы можете выбрать этап в меню')
...
async def process_successful_promotion_payment(message: types.Message, state: FSMContext):
    ...
    if resp.get('paid'):
        ...
        await message.answer('Продвижение успешно заказано до {date}'.format(date=resp_detail.get('end_date')))
        await state.finish()
