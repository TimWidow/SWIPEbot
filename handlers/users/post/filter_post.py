...
async def save_filter_name(message: types.Message, state: FSMContext):
    ...
    if data.get('path'):
        data.pop('path')
    if not data:
        await message.answer('Нет выбранных опций')
        return
    await message.answer('Добавьте имя для фильтра')
...
async def save_filter(message: types.Message, state: FSMContext):
    ...
    resp, status = await Conn.post(REL_URLS['filters'], user_id=message.from_user.id,
                                   data=data)
    if status == 201:
        await message.answer('Фильтры успешно сохранены')
        data = await state.get_data()
        await state.finish()
        ...
        await message.answer('Подробный код ошибки: {code}'.format(code=resp))
...
async def go_to_price(message: types.Message, state: FSMContext):
    data = await state.get_data()
    text = 'Текущая цена <b>{price__gte}:{price__lte}</b>'.format(price__gte=data.get('price__gte', translate('Не указано')),
                                                                  price__lte=data.get('price__lte', translate('Не указано')))
    await message.answer(text)
    await message.answer('Введите новое значение цены для поиска')
    await FilterPost.PRICE.set()


@dp.message_handler(Text(equals=['Перейти к площади', 'Go to area']), state=FilterPost)
async def go_to_area(message: types.Message, state: FSMContext):
    data = await state.get_data()
    text = 'Текущая площадь <b>{square__gte}:{square__lte}</b>'.\
        format(square__gte=data.get('flat__square__gte', 'Не указано'),
               square__lte=data.get('flat__square__lte', 'Не указано'))
    await message.answer(text)
    await message.answer('Введите новое значение площади для поиска')
    await FilterPost.AREA.set()


@dp.message_handler(Text(equals=['Перейти к городу', 'Go to city']), state=FilterPost)
async def go_to_city(message: types.Message, state: FSMContext):
    data = await state.get_data()
    text = 'Текущая город <b>{city}</b>'.format(city=data.get('house__city', 'Не указано'))
    await message.answer(text)
    await message.answer('Введите новый город для поиска')
    await FilterPost.CITY.set()


@dp.message_handler(Text(equals=['Перейти к состоянию квартиры', 'Go to flat state']), state=FilterPost)
async def go_to_state(message: types.Message, state: FSMContext):
    data = await state.get_data()
    labels = filter_post.labels
    text = 'Выбранное состояние <b>{state}</b>'.format(state=labels.get(data.get('flat__state'), 'Не указано'))
    await message.answer(text)
    await message.answer('Выберите новое состояние для поиска',
                         reply_markup=await filter_post.get_filter_post_state_keyboard())
    await FilterPost.STATE.set()


@dp.message_handler(Text(equals=['Перейти к планировке', 'Go to plan']), state=FilterPost)
async def go_to_plan(message: types.Message, state: FSMContext):
    data = await state.get_data()
    labels = filter_post.labels
    text = 'Выбранная планировка <b>{plan}</b>'.format(plan=labels.get(data.get('flat__plan'), 'Не указано'))
    await message.answer(text)
    await message.answer('Выберите новую планировку для поиска',
...
async def go_to_territory(message: types.Message, state: FSMContext):
    data = await state.get_data()
    labels = filter_post.labels
    text = 'Выбранная территория <b>{terr}</b>'.format(terr=labels.get(data.get('house__territory'), 'Не указано'))
    await message.answer(text)
    await message.answer('Выберите новую территорию для поиска',
...
async def go_to_filter(message: types.Message, state: FSMContext):
    ...
    await message.answer(text)
    await message.answer('Подтвердите', reply_markup=await filter_post.get_filter_post_confirm_keyboard())
...
async def filter_post_handler(message: types.Message, state: FSMContext):
    ...
    await message.answer(text='Заполните форму для фильтрации', reply_markup=keyboard)
    await state.update_data(path=path)
    log.info({'path': path})
    await message.answer(text='Укажите цену в формате от:до')
    await FilterPost.PRICE.set()
...
async def filter_price(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer('Введите площадь в формате "от:до"')
        await FilterPost.AREA.set()
    except ValueError:
        await message.answer('Неправильный формат ввода')


@dp.message_handler(state=FilterPost.AREA)
async def filter_square(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        await message.answer('Неправильнй формат ввода')
...
async def filter_city(message: types.Message, state: FSMContext):
    ...
    await message.answer('Укажите желаемое состояние квартиры',
                         reply_markup=await filter_post.get_filter_post_state_keyboard())
    await FilterPost.STATE.set()
...
async def filter_state(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.answer()
    await call.message.answer('Выберите желаемый тип территории',
                              reply_markup=await filter_post.get_filter_post_territory_keyboard())
    await FilterPost.TERR.set()
...
async def filter_state(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.answer()
    await call.message.answer('Выберите желаемый тип планировки',
                              reply_markup=await filter_post.get_filter_post_plan_keyboard())
    await FilterPost.PLAN.set()
...
async def filter_state(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer('Подтвердите', reply_markup=await filter_post.get_filter_post_confirm_keyboard())
    await call.answer()
    await FilterPost.FILTERING.set()
...
async def filter_state(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if value == 'YES':
        ...
        await state.finish()
        params['path'] = path
        await call.message.answer('Возврат', reply_markup=keyboard)
        await state.update_data(**params)
    else:
        await call.message.answer('Используйте меню чтобы выбрать этап фильтрации')
    await call.answer()
