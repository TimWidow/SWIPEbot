...
async def go_to_email(message: types.Message):
    await message.answer(('Добавьте фото профиля'))
    await EditUserDate.PHOTO.set()


@dp.message_handler(Text(equals=['Перейти к роли', 'Go to role']), state=EditUserDate)
async def go_to_email(message: types.Message):
    await message.answer(('Выберите вашу пользовательскую роль'), reply_markup=edit_user_role_keyboard)
...
async def edit_data(message: types.Message, state: FSMContext):
    ...
    await message.answer(('Текущие данные:\n' + text))
    await message.answer(('Введите имя и фамилию через пробел.\n' +
...
async def set_full_name(message: types.Message, state: FSMContext):
    ...
    data = await state.get_data()
    await message.answer(('Введите ваш email.\n' +
                           'Сейчас: <b>{email}</b>').format(email=data['user_info'].get('email', ('Не указано'))))
    await EditUserDate.EMAIL.set()
...
async def set_email(message: types.Message, state: FSMContext):
    ...
    if '@' not in answer or '.' not in answer:
        await message.answer(('Неверный формат электронной почты'))
        return
    await update_state(state, answer, 'email', 'edit_data')
    data = await state.get_data()
    await message.answer(('Выбериет вашу пользовательскую роль.\n' +
                           'Сейчас: <b>{role}</b>').format(role=data['user_info'].get('role', ('Не указано'))),
...
async def edit_role(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.answer()
    await call.message.answer(('Добавьте фото профиля'))
    await EditUserDate.PHOTO.set()
...
async def get_confirm_user(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if value == 'YES':
        ...
        await call.answer(('Вы можете выбрать нужный этап через меню'))
