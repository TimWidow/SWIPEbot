...
async def save_house(message: types.Message, state: FSMContext):
    ...
    if all(key in house_data for key in keys):
        await message.answer('Подтверждаете?',
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Введите название дома')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['create_house']['name']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    ...
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['create_house']['city']))
    await CreateHouse.CITY.set()


@dp.message_handler(Text(equals=['Перейти к адресу', 'Go to address']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Введите адрес дома')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['create_house']['address']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите технологию строительства', reply_markup=create_house.tech_keyboard)
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['tech_display']))
    await CreateHouse.TECH.set()


@dp.message_handler(Text(equals=['Перейти к территории', 'Go to territory']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите тип территории', reply_markup=create_house.terr_keyboard)
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['territory_display']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите предпочитаемые способы платежа')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['payment_options_display']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    ...
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['create_house']['description']))
    await CreateHouse.DESCRIPTION.set()


@dp.message_handler(Text(equals=['Перейти к статусу', 'Go to status']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите статус дома')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['status_display']))
    await CreateHouse.ROLE.set()


@dp.message_handler(Text(equals=['Перейти к типу', 'Go to type']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Введите тип дома')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['type_display']))
    await CreateHouse.TYPE.set()


@dp.message_handler(Text(equals=['Перейти к классу дома', 'Go to house class']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите класс дома')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['house_class_display']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Введите расстояние до моря')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['create_house']['distance_to_sea']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    ...
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['create_house']['ceiling_height']))
    await CreateHouse.HEIGHT.set()


@dp.message_handler(Text(equals=['Перейти к газопроводу', 'Go to gas']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите тип газопровода')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['gas_display']))
    await CreateHouse.GAS.set()


@dp.message_handler(Text(equals=['Перейти к отоплению', 'Go to heating']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите тип отопления')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['heating_display']))
    await CreateHouse.HEATING.set()


@dp.message_handler(Text(equals=['Перейти к электричеству', 'Go to electricity']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите тип электроподачи')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['electricity_display']))
    await CreateHouse.ELECTRICITY.set()


@dp.message_handler(Text(equals=['Перейти к канализации', 'Go to sewerage']), state=CreateHouse)
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите тип канализации')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['sewerage_display']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Выберите тип водоснабжения')
    state_data = await state.get_data()
    if state_data.get('house_info'):
        await message.answer('Сейчас: {value}'.format(value=state_data['house_info']['water_supply_display']))
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Есть ли в вашем доме(или рядом с ним) спортивная площадка?',
                         reply_markup=await create_house.get_advantages_keyboard('add_playground'))
    state_data = await state.get_data()
    if state_data.get('house_info'):
        text = 'Есть' if state_data['create_house']['playground'] else 'Нет'
        await message.answer(text)
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Есть ли в вашем доме(или рядом с ним) парковка?',
                         reply_markup=await create_house.get_advantages_keyboard('add_car_park'))
    state_data = await state.get_data()
    if state_data.get('house_info'):
        text = 'Есть' if state_data['create_house']['car_park'] else 'Нет'
        await message.answer(text)
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Есть ли в вашем доме(или рядом с ним) магазин?',
                         reply_markup=await create_house.get_advantages_keyboard('add_shop'))
    state_data = await state.get_data()
    if state_data.get('house_info'):
        text = 'Есть' if state_data['create_house']['shop'] else 'Нет'
        await message.answer(text)
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Есть ли в вашем доме(или рядом с ним) детская площадка?',
                         reply_markup=await create_house.get_advantages_keyboard('add_child_playground'))
    state_data = await state.get_data()
    if state_data.get('house_info'):
        text = 'Есть' if state_data['create_house']['child_playground'] else 'Нет'
        await message.answer(text)
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Есть ли в вашем доме лифт?',
                         reply_markup=await create_house.get_advantages_keyboard('add_elevator'))
    state_data = await state.get_data()
    if state_data.get('house_info'):
        text = 'Есть' if state_data['create_house']['elevator'] else 'Нет'
        await message.answer(text)
...
async def go_to_house_name(message: types.Message, state: FSMContext):
    await message.answer('Есть ли в вашем доме охрана?',
                         reply_markup=await create_house.get_advantages_keyboard('add_security'))
    state_data = await state.get_data()
    if state_data.get('house_info'):
        text = 'Есть' if state_data['create_house']['security'] else 'Нет'
        await message.answer(text)
...
async def edit_house(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Редактируйте информацию\n' +
...
async def add_house(message: types.Message, state: FSMContext):
    ...
    await message.answer(('Введите название дома'), reply_markup=keyboard)
...
async def house_name(message: types.Message, state: FSMContext):
    ...
    if state_data.get('house_info'):
        await message.answer(('Текущий город: {city}').format(city=state_data['create_house']['name']))
    await message.answer(('Введите город'))
    await CreateHouse.CITY.set()
...
async def house_city(message: types.Message, state: FSMContext):
    ...
    if state_data.get('house_info'):
        await message.answer(('Текущий адрес {address}').format(address=state_data['create_house']['address']))
...
async def house_city(message: types.Message, state: FSMContext):
    ...
    if state_data.get('house_info'):
        await message.answer(('Сейчас: {tech}').format(tech=state_data['house_info']['tech_display']))
    await message.answer(('Выберите технологию строительства дома'),
...
async def house_tech(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {terr}').format(terr=state_data['house_info']['territory_display']))
    await call.message.answer(('Выберите тип территории'), reply_markup=create_house.terr_keyboard)
    await CreateHouse.TERR.set()
    await call.answer()
...
async def house_terr(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['house_info']['payment_options_display']))
    await call.message.answer(('Выберите предпочитаемый вариант оплаты'),
...
async def house_payment(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['create_house']['description']))
    await call.message.answer(('Дайте описание дома'))
    await CreateHouse.DESCRIPTION.set()
    await call.answer()
...
async def house_city(message: types.Message, state: FSMContext):
    ...
    await message.answer(('<b>Далее идут опциональные параметры. Можете их пропустить.</b>\n' +
...
async def house_status(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Дайте тип дома'), reply_markup=create_house.type_keyboard)
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['house_info']['type_display']))
    await CreateHouse.TYPE.set()
    await call.answer()
...
async def house_type(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Дайте класс дома'), reply_markup=create_house.house_class_keyboard)
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['house_info']['house_class_display']))
    await CreateHouse.HOUSE_CLASS.set()
    await call.answer()
...
async def house_house_class(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Введите расстояние до моря. Или пропустите если такового нет'))
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['create_house']['distance_to_sea']))
    await CreateHouse.SEA.set()
    await call.answer()
        ...
        try:
            ...
            if state_data.get('house_info'):
                await message.answer(
                    ('Сейчас: {value}').format(value=state_data['create_house']['ceiling_height']))
            await CreateHouse.HEIGHT.set()
        except ValueError:
...
async def house_height(message: types.Message, state: FSMContext):
    if message.text:
        ...
        try:
            ...
            state_data = await state.get_data()
            await message.answer(('Выберите тип газопровода'), reply_markup=create_house.gas_keyboard)
            if state_data.get('house_info'):
                await message.answer(
                    ('Сейчас: {value}').format(value=state_data['house_info']['gas_display']))
            await CreateHouse.GAS.set()
        except ValueError:
            await message.answer(('Введите число'))
...
async def house_gas(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Выберите тип отопления'), reply_markup=create_house.heating_keyboard)
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['house_info']['heating_display']))
    await CreateHouse.HEATING.set()
    await call.answer()
...
async def house_heating(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Выберите тип электропровода'), reply_markup=create_house.electricity_keyboard)
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['house_info']['electricity_display']))
    await CreateHouse.ELECTRICITY.set()
    await call.answer()
...
async def house_electricity(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Выберите тип канализации'), reply_markup=create_house.sewerage_keyboard)
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['house_info']['sewerage_display']))
    await CreateHouse.SEWERAGE.set()
    await call.answer()
...
async def house_tech(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Выберите тип водопровода'), reply_markup=create_house.water_supply_keyboard)
    if state_data.get('house_info'):
        await call.message.answer(('Сейчас: {value}').format(value=state_data['house_info']['water_supply_display']))
    await CreateHouse.WATER.set()
    await call.answer()
...
async def house_water(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Далее идут дополнительные параметры'))
    await call.message.answer(('Есть ли в вашем доме(или рядом с ним) спортивная площадка?'),
                              reply_markup=await create_house.get_advantages_keyboard('add_playground'))
    if state_data.get('house_info'):
        text = ('Есть') if state_data['house_info']['playground'] else ('Нет')
        await call.message.answer(text)
    await CreateHouse.PLAYGROUND.set()
    await call.answer()
...
async def house_playground(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Есть ли в вашем доме(или рядом с ним) парковка?'),
                              reply_markup=await create_house.get_advantages_keyboard('add_car_park'))
    if state_data.get('house_info'):
        text = ('Есть') if state_data['house_info']['car_park'] else ('Нет')
        await call.message.answer(text)
    await CreateHouse.CAR_PARK.set()
    await call.answer()
...
async def house_car_park(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Есть ли в вашем доме(или рядом с ним) магазин?'),
                              reply_markup=await create_house.get_advantages_keyboard('add_shop'))
    if state_data.get('house_info'):
        text = ('Есть') if state_data['house_info']['shop'] else ('Нет')
        await call.message.answer(text)
    await CreateHouse.SHOP.set()
    await call.answer()
...
async def house_shop(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Есть ли в вашем доме(или рядом с ним) детская площадка?'),
                              reply_markup=await create_house.get_advantages_keyboard('add_child_playground'))
    if state_data.get('house_info'):
        text = ('Есть') if state_data['house_info']['child_playground'] else ('Нет')
        await call.message.answer(text)
    await CreateHouse.CHILD_PLAYGROUND.set()
    await call.answer()
...
async def house_child_playground(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Есть ли в вашем доме лифт?'),
                              reply_markup=await create_house.get_advantages_keyboard('add_elevator'))
    if state_data.get('house_info'):
        text = ('Есть') if state_data['house_info']['high_speed_elevator'] else ('Нет')
        await call.message.answer(text)
    await CreateHouse.ELEVATOR.set()
    await call.answer()
...
async def house_elevator(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Есть ли в вашем доме охрана?'),
                              reply_markup=await create_house.get_advantages_keyboard('add_security'))
    if state_data.get('house_info'):
        text = ('Есть') if state_data['house_info']['security'] else ('Нет')
        await call.message.answer(text)
    await CreateHouse.SECURITY.set()
    await call.answer()
...
async def house_security(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    ...
    await call.message.answer(('Добавьте изображение дома'))
    await CreateHouse.IMAGE.set()
    await call.answer()
...
async def get_image(message: types.Message, state: FSMContext):
    ...
    await message.answer(('Подтверждаете?'), reply_markup=create_house.confirm_keyboard)
    await CreateHouse.SAVE.set()
    ...
    if value:
        ...
        with open(image_path, 'rb') as rb_image:
            ...
            if data.get('house_info'):
                ...
                if not resp.get('id'):
                    await call.answer(('Произошла ошибка. Повторите попытку'))
                else:
                    await call.answer(('Информация о доме изменена'), show_alert=True)
                    new_dict = {}
                    for key, value in data.items():
                        if key not in keys_to_delete:
                            new_dict[key] = value
                ...
                if status == 201:
                    await call.answer('Дом создан', show_alert=True)
                    new_dict = {}
                    for key, value in data.items():
                        if key not in keys_to_delete:
                            new_dict[key] = value
                    ...
                    await call.answer('Произошла ошибка. Повторите попытке')
                    for key, value in resp.items():
                        log.info('Create house')
                        log.info(f'{key} - {value}')
    else:
        await call.answer('Вы можете выбрать нужные этап через меню')
