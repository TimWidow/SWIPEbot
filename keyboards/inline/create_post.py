...
async def get_payment_options() -> InlineKeyboardMarkup:
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton('Ипотека', callback_data=ITEM_CB.new(action='add_payment',
                                                                                    value='MORTGAGE'))
               ).add(
...
async def get_communication_keyboard() -> InlineKeyboardMarkup:
    markup = InlineKeyboardMarkup()
    markup.add(InlineKeyboardButton('Звонок', callback_data=ITEM_CB.new(action='add_comm',
                                                                                   value='CALL'))
               ).add(
        InlineKeyboardButton('Сообщение', callback_data=ITEM_CB.new(action='add_comm',
                                                                               value='MESSAGE'))
    ).row(
        InlineKeyboardButton('Звонок + сообщение', callback_data=ITEM_CB.new(action='add_comm',
                                                                                        value='BOTH'))
    )
    return markup
...
async def get_create_post_confirm_keyboard() -> InlineKeyboardMarkup:
    markup = InlineKeyboardMarkup()
    markup.add(
        InlineKeyboardButton(text='Подтверждаю', callback_data=ITEM_CB.new(action='create_confirm',
                                                                                      value='YES')),
        InlineKeyboardButton(text='Ещё подумаю', callback_data=ITEM_CB.new(action='create_confirm',
                                                                                      value='NO'))
    )
    return markup
